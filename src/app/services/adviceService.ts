...
export class AdviceService {
    ...
    readonly GLOBALRISK: AdviceTable = {
        ...
        // [AdviceCode.Tolstakov1]: [
        //     {
        //         description: "Диверсификация по странам мира позволяет снизить риск инвестирования в экономику только одной страны. Возможно, доля ETF на акции отдельной страны увеличилась вследствие резких движений на рынке."
        //     }
        // ],
        // [AdviceCode.Tolstakov2]: [
        //     {
        //         description: "Диверсификация портфеля по классам активов с учётом риск-профилирования позволяет достичь большей доходности на длительных промежутках времени. Возможно, доля облигаций в Вашем портфеле увеличилась из-за просадки акций, на это следует обратить внимание при ребалансировке портфеля."
        //     }
        // ],
        // [AdviceCode.Tolstakov3]: [
        //     {
        //         description: "Доли ETF менее 5% малоэффективны, т.к. не оказывают серьёзного влияния на портфель. Возможно, доля ETF на акции отдельной страны уменьшилась вследствии резких движений на рынке."
        //     }
        // ],
        // [AdviceCode.Tolstakov4]: [
        //     {
        //         description: "Выбор акций отдельных компаний несёт в себе больше риска, чем выбор индексных фондов (ETF на рынки акций разных стран). Включение в портфель индексных фондов позволяет минимизировать риски выбора акций одной компании, достичь хорошей диверсификации портфеля с минимальными издержками для инвестора.По статистике, 80+% активно управляемых фондов проигрывают индексным пассивным фондам в долгосрочном (5+ лет) периоде."
        //     }
        // ],
        // [AdviceCode.Tolstakov5]: [
        //     {
        //         description: "Выбор акций отдельных компаний несёт в себе больше риска, чем выбор индексных фондов (ETF на рынки акций разных стран). Включение в портфель индексных фондов позволяет минимизировать риски выбора акций одной компании, достичь хорошей диверсификации портфеля с минимальными издержками для инвестора.По статистике, 80+% активно управляемых фондов проигрывают индексным пассивным фондам в долгосрочном (5+ лет) периоде."
        //     }
        // ],
        // [AdviceCode.Tolstakov6]: [
        //     {
        //         description: "Выбор акций отдельных компаний несёт в себе больше риска, чем выбор индексных фондов (ETF на рынки акций разных стран). Включение в портфель индексных фондов позволяет минимизировать риски выбора акций одной компании, достичь хорошей диверсификации портфеля с минимальными издержками для инвестора.По статистике, 80+% активно управляемых фондов проигрывают индексным пассивным фондам в долгосрочном (5+ лет) периоде."
        //     }
        // ],
        // [AdviceCode.Tolstakov7]: [
        //     {
        //         description: "Включение в портфель акций развивающихся рынков (РФ, Китай) в виде ETF (индексных фондов) или отдельных акций российских эмитентов увеличит потенциальную доходность портфеля в длительной перспективе, учитывая фундаментальную недооценённость экономик этих стран по показателям Р/Е, САРЕ относительно других стран."
        //     }
        // ],
        // [AdviceCode.Tolstakov8]: [
        //     {
        //         description: "Включение в портфель акций развивающихся рынков (РФ, Китай) в виде ETF (индексных фондов) или отдельных акций российских эмитентов увеличит потенциальную доходность портфеля в длительной перспективе, учитывая фундаментальную недооценённость экономик этих стран по показателям Р/Е, САРЕ относительно других стран."
        //     }
        // ],
        // [AdviceCode.Tolstakov9]: [
        //     {
        //         description: "Включение в портфель акций развивающихся рынков (РФ, Китай) в виде ETF (индексных фондов) или отдельных акций российских эмитентов, безусловно, увеличит потенциальную доходность портфеля в длительной перспективе, учитывая фундаментальную недооценённость экономик этих стран по показателям Р/Е, САРЕ относительно других стран."
        //     }
        // ]
    };
    // tslint:enable
    ...
    getProblem(adviceUnicCode: AdviceUnicCode): string {
        switch (adviceUnicCode.code) {
            ...
            // case AdviceCode.Tolstakov1:
            //     return `В Вашем портфеле доля ETF на акции одной страны выше 20%`;
            // case AdviceCode.Tolstakov2:
            //     return `В Вашем портфеле доля облигаций выше 40%`;
            // case AdviceCode.Tolstakov3:
            //     return `В Вашем портфеле есть ETF с долей менее 5%`;
            // case AdviceCode.Tolstakov4:
            //     return `В Вашем портфеле доля отдельных акций более 15%`;
            // case AdviceCode.Tolstakov5:
            //     return `В Вашем портфеле доля отдельных акций более 20%`;
            // case AdviceCode.Tolstakov6:
            //     return `В Вашем портфеле доля отдельных акций более 25%`;
            // case AdviceCode.Tolstakov7:
            //     return `В Вашем портфеле нет акций развивающихся рынков, или их доля менее 10%`;
            // case AdviceCode.Tolstakov8:
            //     return `В Вашем портфеле нет акций развивающихся рынков, или их доля менее 15%`;
            // case AdviceCode.Tolstakov9:
            //     return `В Вашем портфеле доля акций развивающихся рынков более 20%`;
        }
